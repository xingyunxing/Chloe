/*
 * Copy from ef core: https://github.com/dotnet/efcore
 * License: MIT
 */
...
namespace Chloe.Query
{
    ...
    internal static class CoreStrings
    {
        ...
        {
            return $"Unhandled expression node '{expressionType}'.";
        }

        public static string SameParameterInstanceUsedInMultipleLambdas(object? parameterName)
        {
            return $"Same parameter '{parameterName}' instance used in multiple lambdas.";
        }

        public static string UnhandledMemberBinding(object? bindingType)
        {
            return $"Unhandled member binding '{bindingType}'.";
        }
    ...
    public sealed class ExpressionEqualityComparer : IEqualityComparer<Expression?>
    {
                    ...
                    case ConstantExpression constantExpression:
                        switch (constantExpression.Value)
                        {
                            ...
                            case IStructuralEquatable structuralEquatable:
                                ...
                                break;
                            case QueryExpression queryExpression:  //QueryObjectExpressionTransformer 把 IQuery 对象替换成 QueryExpression 常量，所以得在这处理一下 QueryExpression 的情况
                                hash.Add(QueryExpressionEqualityComparer.Instance.GetHashCode(queryExpression));
                                break;
                            default:
        ...
        private struct ExpressionComparer
        {
            ...
            private static bool CompareConstant(ConstantExpression a, ConstantExpression b)
            {
                ...
                if (v1 is QueryExpression queryExpression1 && v2 is QueryExpression queryExpression2)
                {
                    //QueryObjectExpressionTransformer 把 IQuery 对象替换成 QueryExpression 常量，所以得在这处理一下 QueryExpression 的情况
                    return QueryExpressionEqualityComparer.Instance.Equals(queryExpression1, queryExpression2);
                }
