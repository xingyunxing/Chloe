    ...
    internal class AggregateQuery : FeatureEnumerable<QueryResult<object>>
    {
        ...
        class Enumerator : IFeatureEnumerator<QueryResult<object>>
        {
            ...
            void Init()
            {
                ...
                List<AggregateQueryPlan<object>> aggQueryPlans = new List<AggregateQueryPlan<object>>(tables.Count);
                ...
                foreach (var group in aggQueryPlans.GroupBy(a => a.QueryModel.Table.DataSource.Name))
                {
                    ...
                    foreach (AggregateQueryPlan<object> aggQueryPlan in group)
                    {
                        var query = new SingleTableAggregateQuery<object>(dbContextPool, aggQueryPlan.QueryModel, this._enumerable._executor);
